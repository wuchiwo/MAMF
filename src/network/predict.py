...
dataset_train = df.iloc[:c.train_size, 1:2]
dataset_test = df.iloc[c.train_size:, 1:2]
dataset_total = pd.concat((dataset_train, dataset_test), axis = 0)

inputs = dataset_total[len(dataset_total) - len(dataset_test) - c.lookback_days:].values
inputs = inputs.reshape(-1,1)
inputs = sc.fit_transform(inputs)

x_test = []
for i in range(c.lookback_days, inputs.size):
    x_test.append(inputs[i-c.lookback_days:i, 0])
