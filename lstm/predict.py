...
train_set = df.loc[:train_size, cfg.feature_keys]
test_set = df.loc[train_size:, cfg.feature_keys]
total_set = pd.concat((train_set, test_set))

inputs = total_set[len(train_set) - cfg.past:].values
inputs = sc.fit_transform(inputs)

x_test = []
for i in range(cfg.past, inputs.shape[0]):
...
predict_set = pd.DataFrame(columns = [cfg.date_key, cfg.predict_key])
...
plt.plot(df.loc[train_size:, cfg.date_key], predict_value,
