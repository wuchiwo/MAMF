...
class Indikeppar:
    ...
    def keppar(self, term=20, ti=10, draw=False):
        for idx, item in self.data.iterrows():
            self.data.loc[idx, 'P'] = 1
            try:
                # log P(A,T) = PI(1+r(A,t))
                for t in range (ti, term):
                    self.data.loc[idx, 'P'] *= (1 + self.data.loc[idx, 'value'] - self.data.loc[idx - t, 'value'])
                    self.data.loc[idx, 'P'] = math.log(self.data.loc[idx, 'P'])
            except:
                pass
        for idx, item in self.data_comparison.iterrows():
            # log P(A,T) = PI(1+r(A,t))
            self.data_comparison.loc[idx, 'P'] = 1
            try:
                for t in range (ti, term):
                    self.data_comparison.loc[idx, 'P'] *= (1 + self.data_comparison.loc[idx, 'value'] - self.data_comparison.loc[idx - t, 'value'])
                    self.data_comparison.loc[idx, 'P'] = math.log(self.data_comparison.loc[idx, 'P'])
            except:
                pass
        #SABt ONLY FOR MAIN DATA
        self.data['SABT'] = self.data['P'] - self.data_comparison['P']
        for idx, item in self.data.iterrows():
            #mABT ONLY FOR MAIN DATA
            self.data.loc[idx, 'mABT'] = 0
            try:
                for t in range(ti, term):
                    self.data.loc[idx, 'mABT'] += self.data.loc[idx - t, 'SABT'] / (term - ti)
            except:
                pass
            #mABTt-1 ONLY FOR MAIN DATA
            self.data.loc[idx, 'mABT-1'] = 0
            try:
                for t in range(ti - 1, term - 1):
                    self.data.loc[idx, 'mABT-1'] += self.data.loc[idx - t, 'SABT'] / (term - ti)
            except:
                pass
            #sABT ONLY FOR MAIN DATA
            self.data.loc[idx, 'sABT'] = 0
            try:
                for t in range(ti, term):
                    self.data.loc[idx, 'sABT'] += (self.data.loc[idx - t, 'SABT'] - self.data.loc[idx - term, 'mABT'])/ term
            except:
                pass
        #tTest, keppar
        try:
            self.data['tTest'] = (self.data['SABT'] - self.data['mABT']) / self.data['sABT']
            self.data['keppar'] = (self.data['mABT'] - self.data['mABT-1'])/ self.data['mABT-1']
        except:
            self.data['tTest'] = 1
            self.data['keppar'] = 1

        for idx, item in self.data.iterrows():
            self.data.loc[idx, 'keppar_Enable'] = -0.3 < self.data.loc[idx, 'keppar'] and 0.3 > self.data.loc[idx, 'keppar']
            ...
            self.data.loc[idx, 'keppar_Sell'] = (self.data.loc[idx, 'tTest'] < 1.8 or \
                (self.data.loc[idx, 'tTest'] < 0 and self.data.loc[idx - 1, 'tTest'] > 0) or \
                (self.data.loc[idx, 'tTest'] > 3 and self.data.loc[idx - 1, 'tTest'] < 3) or \
                (self.data.loc[idx, 'tTest'] < -3 and self.data.loc[idx - 1, 'tTest'] > -3)) * self.data.loc[idx, 'keppar_Enable']
...
class Indicator:
    ...
    def bollinger(self, sigma=2, show=False):
        ...
        for idx, item in self.data.iterrows():
            ...
            self.data.loc[idx, 'Bollinger_Buy'] = (self.data.loc[idx, 'value'] <= self.data.loc[idx, 'Lower Band']) * self.data.loc[idx, 'Trend_bull']
            self.data.loc[idx, 'Bollinger_Sell'] = (self.data.loc[idx, 'value'] >= self.data.loc[idx, 'Upper Band']) * self.data.loc[idx, 'Trend_bear']
        if show == True:
    ...
    def trend(self,ma=50):
        for idx, item in self.data.iterrows():
            try:
                self.data.loc[idx, 'Trend_bull'] = self.data.loc[idx, str(ma)+'MA'] > self.data.loc[idx - 1, str(ma)+'MA']
                self.data.loc[idx, 'Trend_bear'] = self.data.loc[idx, str(ma)+'MA'] < self.data.loc[idx - 1, str(ma)+'MA']
            except:
